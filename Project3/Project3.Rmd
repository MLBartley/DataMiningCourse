...
The "best" unpruned tree created with the training data was chosen to be the tree that minimizes misclassification rate. Using these rates we found that within pruned tree we observed a prediciton error rate in the training data of 2.89\% and a prediciton error rate in cross-validation of 2.96\%. We consider minimizing  misclassification rates to be vital with these car condition classification  data as the consequences of purchasing/driving in an unacceptable car includes a risk to one's personal safety. 
...
The confusion matrices below show the accuracy of the tree's predicitons for the training (top) and testing (bottom) datasets. 
