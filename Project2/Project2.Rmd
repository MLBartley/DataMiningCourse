...
for ( k in 2:10) {
  km = kcca(p2d_train, k = k, kccaFamily("kmeans"))
  
  pred_train <- predict(km)
  pred_test <- predict(km, newdata=p2d_test)
  
  accurate[k] <- round(sum(diag(prop.table(table(p2d_testlabels, pred_test)))), 2) * 100
}
...
ct.km <- table(p2d_testlabels, pred_test)
...
p2d_pca_test <- project2data.new[-train_id, -8]
p2d_pca_train <- project2data.new[train_id, -8]
p2d_pca_testlabels <- project2data.new[-train_id, 8]
p2d_pda_trainlabels <- project2data.new[train_id, 8]

```


```{r, echo =FALSE}
km <- kmeans(project2data.new[, -8], 3)
...
```{r}
knn <- knn(train = p2d_train, test = p2d_test, cl = p2d_trainlabels, k = 3)

knn
summary(knn)

merge <- data.frame(knn, p2d_testlabels)
names(merge) <- c("Predicted Label", "Observed Label")

merge

gmodels::CrossTable(x = p2d_testlabels, y = knn, prop.chisq = FALSE)
```
```{r include=FALSE}
